...
static __always_inline long process_rule(u32 idx, void* data)
{
    ...
    // Game payload signature check -- only reached once every header-level
    // criterion above already matched, so this is the last (priciest)
    // check, same ordering principle as everything else in this function.
    if (filter->do_game && !payload_matches_game(filter->game_id, ctx->payload, ctx->data_end))
    {
        return 0;
    }
