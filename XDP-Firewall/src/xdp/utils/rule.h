...
struct rule_ctx
{
    ...
    // First byte after the transport header (UDP: right after udphdr; TCP:
    // right after the header including options) -- used by the `game`
    // filter option's payload signature check (see xdp/utils/games.h).
    // NULL when the protocol is ICMP/unrecognized.
    void *payload;
    void *data_end;
