...
int parse_cfg(config__t *cfg, const char* data, config_overrides_t* overrides)
{
    ...
    if (setting && config_setting_is_list(setting))
    {
        for (int i = 0; i < config_setting_length(setting); i++)
        {
            ...
            if (config_setting_lookup_string(filter_cfg, "game", &game) == CONFIG_TRUE)
            {
                filter->game = strdup(game);
            }
    ...
    if (filters)
    {
        for (int i = 0; i < MAX_FILTERS; i++)
        {
            ...
            if (filter_cfg)
            {
                ...
                if (filter->game)
                {
                    config_setting_t* game = config_setting_add(filter_cfg, "game", CONFIG_TYPE_STRING);
                    config_setting_set_string(game, filter->game);
                }
